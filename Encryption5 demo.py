        ...
        if (accName,) not in account:
            ...
            if question < 1:
                ...
                if len(after_d)<5:
                     ...
                     frame.place(relx=0.80, rely= 0.14)
                     frame.place(relheight=0.06, relwidth=0.23)          
                     create_Button = ttk.Button(frame,  image = create_B, style = 'A.TButton',
                     command = goToCreate,state=NORMAL)          
                     create_Button.place(relx=0.00, rely=0.11)
                     create_Button.place(relheight=0.90, relwidth=0.54)
                     ToolTip.ToolTip(create_Button, 'Create, encrypt and store a new account \n\
...
Do you want to replace it?',
                  ...
                  (ID, accName, accountID, salt, password, secretQ, SecretQans, Account_Notes))
                        conn.commit()
                        num = num +1
                        #print( len(account_check), ' len(account_check)')
                        if len(account_check)<5:
...
',  
            ...
            elif len(account_check)>=5:
                frame =ttk.Frame(root,style = "E.TFrame")
                frame.place(relx=0.80, rely= 0.14)
                frame.place(relheight=0.06, relwidth=0.23)          
                create_Button = ttk.Button(frame,  image = create_B, style = 'A.TButton',
                command = goToCreate,state=DISABLED)          
                create_Button.place(relx=0.00, rely=0.11)
                create_Button.place(relheight=0.90, relwidth=0.54)
                ToolTip.ToolTip(create_Button, 'Create, encrypt and store a new account \n\
Keyboard shortcut: "Alt+C", but must enter Master Password first')
                e_message = messagebox.askokcancel(message='En-Cryption Demo Version has '+ str(len(account_check))+
' accounts on file. \n\
Please upgrade to the full version for unlimited account creation.',title='Accounts on file', parent = window1)
                if e_message ==1:
                    url = "http://en-cryption.squarespace.com/shop/"
                    webbrowser.open(url)
                else:
                    e_message = messagebox.askokcancel(message='You have successfully encrypted the '
                                            +accName+' account.\n\
...
',  
                ...
                for row in cursor.execute("SELECT accName FROM User_Accounts "):
                    check.append(row)
                #print( len(check), ' len(check) else below')
                if len(check)<5:
                    e_message = messagebox.askokcancel(message='You have successfully encrypted the '
                                            +accName+' account.\n\
To view this data in encrypted format, press OK.\n\
...
Keyboard shortcut: "Alt+C", but must enter Master Password first')
                    e_message = messagebox.askokcancel(message='You have successfully encrypted the '
                                                       +accName+' account. \n\
En-Cryption Demo Version has '+ str(len(check))+
' encrypted accounts on file now. \n\
Please upgrade to the full version for unlimited account creation.',title='Accounts on file', parent = window1)
                    if e_message ==1:
                        url = "http://en-cryption.squarespace.com/shop/"
                        webbrowser.open(url)
                        ...
                        e_message = messagebox.askokcancel(message='To view '+accName+' data in encrypted format, press OK. \n\
Else, CANCEL to continue.', title='Account Encrypted', parent = window1)
                        if e_message < 1:
                            window1.destroy()
                        else:
 ...
 '-', '_', '=', '+', '{', '[', '}', ']', '|', '/', '"', ':', ';', ',', '<', '.', '>', '?', "'", '~', '`']
    ...
    for item in check_list:
        ...
        if len(check) == len(check_list) :
            ...
            if length <9:
                 v2 = ttk.Label(root,style = "C.TLabel", width=127,
                           text = 'password is too short', anchor = CENTER,
                       relief = 'raised',font = "Palatino 12 bold ",
...
def enter(*args): # this creates the buttons that allow access to
    ...
    def demo_check(*args):
        ...
        try:
            ...
            print('none')
        if len(account_check)<=4:# number of accounts         
            frame =ttk.Frame(root,style = "E.TFrame")
            frame.place(relx=0.80, rely= 0.14)
            frame.place(relheight=0.06, relwidth=0.23)          
            create_Button = ttk.Button(frame,  image = create_B, style = 'A.TButton',
...
Keyboard shortcut: "Alt+C", but must enter Master Password first')
            e_message = messagebox.askokcancel(message='En-Cryption Demo Version has '+ str(len(account_check))+
' encrypted accounts on file. \n\
Please upgrade to the full version for unlimited account creation.',title='Accounts on file', parent = root)
            if e_message ==1:
                url = "http://en-cryption.squarespace.com/shop/"
                webbrowser.open(url)
    ...
    m = mass_pass.get()    
    mass_pass.set(m)  
    images = [delete_B, view_B, help_B, about_B, quit_B]
    commands = [goToDelete, goToView, goToHelp,
                goToAbout, goToQuit]
    tool = [
...
# all window images
main_win = PhotoImage(file='Blue Final Demo.gif')
...
upgrade = PhotoImage(file='Upgrade Button.gif')
...
upgrade_B = ttk.Button(root,  image =upgrade,
                        style = 'A.TButton', command = upgrade_it)
upgrade_B.place(relx=0.605, rely=0.722)
upgrade_B.place(relheight=0.04, relwidth=0.15)
